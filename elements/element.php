...
if ( ! function_exists( 'content_meta_complete_date' ) ) {
	function content_meta_complete_date() {
		return '<time itemprop="dateCreated datePublished" class="meta-date updated"  datetime="' . esc_attr(date(DATE_W3C, get_the_time('U'))) . '">' .get_the_date('j F Y'). '</time>';
	}
}
		...
		if(has_post_format('video') && $video){
			...
			if (has_post_thumbnail()) {
				...
				if (get_post_format() =='aside' && $featured=='featured'){		
					...
					$out .= '<section id="1" class="parallax" style="background-image: url(';
