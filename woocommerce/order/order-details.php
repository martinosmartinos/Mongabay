					...
					<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
						<td class="product-name">
							<?php
								...
								if ( $item_meta->meta ) {
									$imeta = $item_meta->get_formatted();
									if (!empty($imeta['picture_id'])) {
										$pic_id = intval($imeta['picture_id']['value']);
										echo '<br/><small><a href="' . get_permalink($pic_id) . '">'.get_the_title($pic_id).'</a></small>';
									}
									else echo '<br/><small>' . nl2br( $item_meta->display( true, true, '_', "\n" ) ) . '</small>';
								}
